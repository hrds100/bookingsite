...
async function provisionWallet(userId: string) {
  try {
    ...
    try {
      jwt = sessionStorage.getItem("particle_jwt");
    } catch {
      /* skip */
    }
    ...
    if (address) {
      ...
      try {
        sessionStorage.removeItem("particle_jwt");
      } catch {
        /* skip */
      }
    }
