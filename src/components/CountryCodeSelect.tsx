...
export default function CountryCodeSelect({ value, onChange }: CountryCodeSelectProps) {
  ...
  return (
    <div data-feature="NFSTAY__COUNTRY_SELECT" className="relative">
      <button
        type="button"
        onClick={() => setOpen(!open)}
        className="flex items-center gap-1.5 h-[41px] px-3 bg-white border border-[#e5e5e5] border-r-0 rounded-l-[10px] text-sm transition-colors shrink-0 hover:bg-[#f5f5f5]"
      >
        <span className="text-base leading-none">{selected.flag}</span>
        <span className="font-medium text-[#0a0a0a]">{selected.dial}</span>
      ...
      {open && (
        <>
          ...
          <div className="absolute top-full left-0 mt-1 w-[260px] bg-white border border-[#e5e5e5] rounded-lg shadow-lg z-50 overflow-hidden">
            ...
            <div className="max-h-[240px] overflow-y-auto">
              {filtered.map((c) => (
                <button
                  ...
                  className={`w-full flex items-center gap-2.5 px-3 py-2 text-sm hover:bg-[#f5f5f5] transition-colors ${
                    c.dial === value ? "bg-[#f5f5f5]" : ""
                  }`}
                >
                  <span className="text-base leading-none">{c.flag}</span>
                  <span className="font-medium text-[#0a0a0a]">{c.dial}</span>
                  <span className="text-[#737373] text-xs ml-auto">{c.name}</span>
                </button>
              ))}
              {filtered.length === 0 && (
                <p className="text-sm text-[#737373] text-center py-4">No results</p>
              )}
            </div>
          </div>
        </>
