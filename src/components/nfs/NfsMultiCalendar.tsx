...
import { ChevronLeft, ChevronRight, CalendarDays, Ban, Unlock } from "lucide-react";
...
export interface NfsMultiCalendarProps {
  ...
  onToggleBlock: (propertyId: string, date: string, block: boolean) => void;
  loading?: boolean;
}
    ...
    <div className="space-y-3" data-feature="NFSTAY__OP_MULTI_CALENDAR">
                    ...
                    {dates.map((date) => {
                      ...
                      return (
                        <div
                          ...
                          onClick={() =>
                            !inRes && onToggleBlock(property.id, dateStr, !blocked)
                          }
                          title={
                            inRes
                              ? undefined
      ...
      <div className="flex items-center gap-4 text-xs text-muted-foreground px-1">
        <span className="flex items-center gap-1">
          <Ban className="w-3 h-3 text-rose-400" />
          Click an available date to block it
        </span>
        <span className="flex items-center gap-1">
