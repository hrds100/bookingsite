...
export default function AdminSystemHealth() {
  ...
  const refresh = useCallback(async () => {
    setLoading(true);
    const map = await runAllChecks(BOOKING_SERVICES);
    setResults(map);
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-2xl font-bold text-foreground mb-1">System Health</h1>
        <p className="text-sm text-muted-foreground">
          Live status of the services that power nfstay.app
        </p>
      </div>
      ...
      {/* ── Booking Flow ────────────────────────────── */}
      <div>
        <h3 className="text-sm font-bold text-foreground mb-4">Booking Flow</h3>
          ...
          return (
            <div key={step.label} className="flex items-center">
              ...
              {i < flow.steps.length - 1 && (
                <span className="text-muted-foreground text-xs px-1">&rarr;</span>
              )}
            </div>
          );
        })}
